...
def analyze_pe_file(filepath):
    ...
    # Read the blob size from the first 4 bytes
    if len(rdata) < 4:
        print("[!] ERROR: .rdata section too small to contain blob size", file=sys.stderr)
        return
    
    blob_size = struct.unpack('<I', rdata[:4])[0]
    print(f"[*] Blob size: {blob_size} bytes", file=sys.stderr)
    
    # Calculate required data length: 4 bytes (size) + blob_size + 16 bytes (key)
    required_length = 4 + blob_size + 16
    if len(rdata) < required_length:
        print(f"[!] ERROR: .rdata section too small. Required: {required_length}, Available: {len(rdata)}", file=sys.stderr)
        return
    
    # Extract encrypted data (starts after the 4-byte size field)
    encrypted_data = rdata[4:4 + blob_size]
    
    # Extract RC4 key (16 bytes immediately after the encrypted blob)
    rc4_key = rdata[4 + blob_size:4 + blob_size + 16]
    
    print(f"[*] Encrypted data length: {len(encrypted_data)} bytes", file=sys.stderr)
    print(f"[*] RC4 key: {rc4_key.hex()}", file=sys.stderr)
